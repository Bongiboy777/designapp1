...
import { useSession, getProviders, signIn,  ClientSafeProvider, LiteralUnion, signOut } from 'next-auth/react'

import Link from 'next/link'
import NavLink from './NavLink'
import { BiDoorOpen } from 'react-icons/bi'
import { getProviderLabel } from '@/app/lib/utils'
        ...
        <div id="mobile-menu " className='lg:hidden'>
          <div className="flex  items-center w-fit bg-blue">
            <button className="" type="button" title="mobile-menu" onClick={() => setShowMenu(!showMenu)}>
              <MdOutlineMenu className="w-[48px] my-auto" />
            </button>
          </div>
          ...
          !session ? (
            ...
            <div className="">
              <div className="flex gap-5 justify-center items-center self-stretch p-2.5 my-auto uppercase max-lg:hidden">
                {
                  providers && Object.values(providers).map(provider => (
                    <div key={provider.id} className="cursor-pointer flex items-center text-center bg-blue-600 text-white font-bold px-2 py-2 rounded-md" onClick={() => signIn(provider.id)}>
              ...
              <div className="relative lg:hidden flex items-center gap-2.5 justify-center">
                ...
                {
                  showUserMenu && (
                    <div className="flex flex-col items-center justify-center gap-5 z-10 absolute right-0 top-10  w-fit text-sm text-zinc-800  shadow-sm border border-zinc-300 rounded-md p-5 bg-white">
                      <h4 className="font-bold text-center underline text-slate-800">Sign In</h4>
                      {providers && Object.values(providers).map(provider => (
                        <div key={provider.id} className="flex items-center self-stretch text-center bg-blue-600 text-white font-bold px-2 py-2 rounded-md" onClick={() => signIn(provider.id)}>
                    {getProviderLabel(provider.id)}
                    <p className="nowrap my-auto cursor-pointer">{provider.id}</p> 
                        </div>
                      ))}
                    </div>
                  )
            ...
            <div className="hidden lg:flex  items-center justify-center gap-5 z-10">
              <div className="relative w-full h-auto" id='notification'>
                <FaBell
                  ...
                  onClick={() => { setShowUserMenu(false);
                     ...
                     return setShowNotificationMenu(!showNotificationMenu); }}
                />
                {
              ...
              <div className="relative w-fit h-auto" id='user'>
                ...
                />
                  ...
                  <AiOutlineUser className='cursor-pointer' size={24} onClick={() => { setShowNotificationMenu(false); return setShowUserMenu(!showUserMenu);}}/>
                )}
                {
                  showUserMenu && (
                    <div className="absolute right-0 top-10  w-fit text-sm text-zinc-800  shadow-sm border border-zinc-300 rounded-md p-5 bg-white">
